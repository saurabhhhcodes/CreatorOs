(function () {
  ...
  function initTopbar() {
    ...
    const iconLight = themeToggle
      ? themeToggle.querySelector(".icon-light")
      : null;
    const iconDark = themeToggle
      ? themeToggle.querySelector(".icon-dark")
      : null;
    ...
    if (themeToggle) {
      themeToggle.addEventListener("click", async () => {
        const nextMode = body.classList.contains("appearance-dark")
          ? "light"
          : "dark";
        document
          .querySelectorAll("#appearance-group .radio-btn")
          .forEach((btn) => {
    ...
    window.addEventListener(
      "creatorosSettingsAppearanceChanged",
      syncThemeIcon,
    );
    syncThemeIcon();
  }
  ...
  function applyPreferences() {
    ...
    localStorage.setItem(
      "creatorosDensity",
      prefs.interfaceDensity || "tactile",
    );
    localStorage.setItem("creatorosMotion", String(!!prefs.motionEffects));
    localStorage.setItem(
      "creatorosAutoSaveLinks",
      String(!!prefs.autoSaveLinks),
    );
  ...
  const sections = ["profile", "security", "billing", "global"].map((id) =>
  ...
  subNavItems.forEach((anchor) => {
    anchor.addEventListener("click", (e) => {
      ...
      if (el) {
        el.scrollIntoView({
          behavior:
            userData.preferences?.motionEffects === false ? "auto" : "smooth",
        });
  ...
  document
    .getElementById("save-pwd-btn")
    .addEventListener("click", async () => {
      if (isGuest)
        return showToast("Contributors cannot change password", true);
      try {
        ...
        document.getElementById("pwd-age-text").textContent =
  ...
  document
    ...
    .addEventListener("click", async () => {
      if (isGuest)
        return showToast("Contributors cannot delete accounts", true);
      if (
        !confirm(
          "Are you sure you want to deactivate your instance? This is permanent.",
        )
      )
        return;
      try {
  ...
  document.querySelectorAll("#density-group .radio-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      document
        .querySelectorAll("#density-group .radio-btn")
        .forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      try {
  ...
  async function refreshBilling() {
    try {
      ...
      document.getElementById("next-invoice-date").textContent =
        billing.nextInvoiceLabel;
      document.getElementById("invoice-est").textContent =
        `Estimated: ${billing.estimatedTotal}`;
      document.getElementById("card-info-text").textContent =
  ...
  document
    .getElementById("billing-modal-close")
    .addEventListener("click", () => {
  ...
  function bindInvoiceRows() {
    document.querySelectorAll("#invoices-table tbody tr").forEach((row) => {
      row.onclick = () => {
        ...
        invoiceModal.dataset.invoiceId =
          row.dataset.invoiceId || cells[1].textContent;
        playSoundCue();
      };
    });
  }
  ...
  document
    .getElementById("invoice-modal-close")
    .addEventListener("click", () => {
  ...
  if (upgradePlanBtn) {
    upgradePlanBtn.addEventListener("click", (e) => {
      e.preventDefault();
      billingModal.querySelector("#billing-modal-title").textContent =
        "Upgrade Plan";
