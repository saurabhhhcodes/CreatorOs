...
function handleFile(file) {
  ...
  uploadBtn.className =
    "mt-6 w-full rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 px-6 py-3 font-bold text-slate-900 cursor-pointer hover:brightness-110 transition-all duration-200";
...
function clearSelection() {
  ...
  uploadBtn.className =
    "mt-6 w-full rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 px-6 py-3 font-bold text-slate-900 opacity-50 cursor-not-allowed transition-all duration-200";
...
function uploadFile(file) {
  ...
  xhr.onload = function () {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      statusEl.innerHTML =
        "<strong>Upload successful!</strong><br>File: " +
        data.filename +
        "<br>Size: " +
