(function () {
  ...
  loadThree(() => {
    ...
    p1.position.set(80, 40, 80);
    scene.add(p1);
    const p2 = new THREE.PointLight(0x818cf8, 0.18, 300);
    p2.position.set(-60, -30, 60);
    scene.add(p2);
    ...
    const spriteTex = new THREE.TextureLoader().load(
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><defs><radialGradient id="g"><stop offset="0" stop-color="white" stop-opacity="1"/><stop offset="1" stop-color="white" stop-opacity="0"/></radialGradient></defs><rect width="100%" height="100%" fill="url(%23g)"/></svg>',
    );
    ...
    const spr1 = new THREE.Sprite(spriteMat);
    spr1.scale.set(220, 120, 1);
    spr1.position.set(0, 10, -40);
    scene.add(spr1);
    const spr2 = new THREE.Sprite(spriteMat);
    spr2.scale.set(160, 80, 1);
    spr2.position.set(-30, -20, -60);
    scene.add(spr2);
    ...
    function tick(now) {
      ...
      tx += (mx - tx) * 0.06;
      ty += (my - ty) * 0.06;
      camera.position.x = tx * 18;
      camera.position.y = ty * -10;
      camera.lookAt(0, 0, 0);
