    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          const message =
            stderr?.trim() ||
            "Unable to fetch Instagram profile via Python provider.";
          return reject(
            new InstagramProfileError("TEMPORARY_FETCH_ERROR", message, 500),
          );
        }
          ...
          return reject(
            new InstagramProfileError(
              ...
              errorPayload.message ||
                "Python provider failed to fetch the profile.",
...
function extractCounts(ogDescription) {
  ...
  const hasCountTokens =
    /Followers/i.test(ogDescription) && /Posts/i.test(ogDescription);
...
async function fetchPublicHtmlProfile(username) {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; CreatorOS/1.0; +https://creatoros.local)",
...
function resolveProvider() {
  const provider = (
    process.env.INSTAGRAM_PUBLIC_PROVIDER || "public_html"
  ).toLowerCase();
