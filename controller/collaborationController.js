  ...
  try {
    ...
    if (!result.success) {
      return renderDashboard(req, res, {
        inviteAcceptError: result.error.issues[0].message,
      });
    }
    ...
    if (!invite) {
      return renderDashboard(req, res, {
        inviteAcceptError: "No invitation found for that token.",
      });
    }

    if (invite.status === "accepted") {
      return renderDashboard(req, res, {
        inviteAcceptMessage: "This invitation has already been accepted.",
      });
    }
