...
const GOOGLE_AUTH_CANCELLED_ERROR =
...
function redirectWithLoginError(res, error) {
  const message =
    error === GOOGLE_AUTH_CANCELLED_ERROR ? error : GENERIC_LOGIN_ERROR;
...
const login = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ success: true, token, user: serializeUser(user) });
});
  ...
  if (!user) {
    ...
    if (wantsHtml(req)) {
      return res.render("resend-verification", {
        success:
          "If that email address is in our system, you'll receive a verification email shortly.",
        error: null,
      });
    }
    return res.status(200).json({
